...
//
// XProperties & XPropertyNames
// extract properties of given TYPE from interface
...
type z0 = XPropertyNames<Types, string>; // should be 'string'
//
//

//
// Types our extension endpoint for our API
//
//
interface Types {
    ["string"]: string,
...
//
// User may extend supported types by
interface GeoCoordinate {
    latitute: number;
    longitude: number;
}

// adding them in its own declaration files
...
type PositionalRequired<B extends string> = { type: B  };
type PositionalOptional<B extends string> = { type: `${B}?` };
type PositionalArray<B extends string> = { type: `${B}[]` };

type Positional<B extends string> = PositionalBase & (PositionalRequired<B> | PositionalOptional<B> | PositionalArray<B>);

type Z1 = Positional<XPropertyNames<Types, string>>;
const testZ1: Z1 = { name: "x", type: "string[]" }
const testZ2: Z1 = { name: "x", type: "string" }
const testZ3: Z1 = { name: "x", type: "string?" }
